...
module tb_cnn;
    ...
    //$readmemh("image_64x64.txt", image_mem); 
    //$readmemh("filter_3x3.txt", filter_mem);
    //     $display("\n##### @%0t, Test Data is ready ####\n", $time);      
...
initial begin
    ...
    always @ (posedge clk) begin			
			...
			conv_done <= (latency_ena[1:0] == 2'b10)?1'b1:1'b0;
    end
    always @ (posedge clk) begin
    ...
    always @ (posedge clk) begin
		...
		begin
			$display("\n##### @%0t, Output data number is over 4096 ####\n",$time ); 
		end
